...
// 			const device =
// 				userAgent?.browser?.name ?? userAgent?.device?.vendor ?? 'Unknown device';
...
// 		const { method, url } = context.switchToHttp().getRequest();
// 		const message = `Outgoing error response - ${error['code'] ?? error['status'] ?? error['statusCode'] ?? 500} - ${method} - ${url}`;
