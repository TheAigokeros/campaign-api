/*
  ...
  - The values [CANCLE] on the enum `CampaignEmailSendType` will be removed. If these variants are still used in the database, this will fail.
  - The values [CANCLE] on the enum `CampaignEmailStatus` will be removed. If these variants are still used in the database, this will fail.

*/
-- AlterEnum
BEGIN;
CREATE TYPE "public"."CampaignEmailSendType_new" AS ENUM ('SCHEDULE', 'TRIGGER', 'CANCEL', 'FAIL');
...
-- AlterEnum
BEGIN;
CREATE TYPE "public"."CampaignEmailStatus_new" AS ENUM ('CANCEL', 'DRAFT', 'SCHEDULE', 'SEND', 'FAIL');
